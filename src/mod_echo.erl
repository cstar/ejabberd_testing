...
init([Host, Opts]) ->
    ...
    Version  = gen_mod:get_opt(version, Opts, disabled),
    ejabberd_router:register_route(MyHost),
    {ok, #state{host = MyHost, version=Version}}.
...
%%--------------------------------------------------------------------
handle_info({route, From, To, Packet}, #state{version=Version} = State) ->
	Packet2 = case From#jid.user of
		"" -> jlib:make_error_reply(Packet, ?ERR_BAD_REQUEST);
		_ -> Packet
	end,
  do_client_version(Version, To, From), % Put 'enabled' to enable it
